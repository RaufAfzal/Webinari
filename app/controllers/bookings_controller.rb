class BookingsController < ApplicationController
    ...
    def create
        ...
        @card = @stripe_service.create_stripe_customer_card(card_token_params,
             @customer)
        
        @amount_to_be_paid = params[:no_of_tickets].to_i * @workshop.registration_fee
        
        @charge = @stripe_service.create_stripe_charge(@amount_to_be_paid, @customer.stripe_customer_id, 
            @card.id, @workshop)
        ...
        redirect_to  workshops_path(@workshop), notice: 'Your tickets has been booked'
